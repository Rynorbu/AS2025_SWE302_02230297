...
**Source Code Repository**: [https://github.com/Rynorbu/SWE302_Practical_7-Performance_Testing_with_k6](https://github.com/Rynorbu/SWE302_Practical_7-Performance_Testing_with_k6)
...
This performance testing project successfully demonstrated the Dog CEO API application's production readiness through comprehensive testing across multiple scenarios. All four test types (Average Load, Spike, Stress, and Soak) were implemented and executed in both local and cloud environments, providing complete performance insights.

**Key Achievements:**

- **Application Performance**: Successfully handles 20 concurrent users with <500ms response times and 0% errors
- **Resilience**: Recovers quickly from traffic spikes and degrades gracefully under stress
- **Stability**: No memory leaks detected during 30-minute endurance testing
- **Technical Skills**: Gained practical experience with k6, performance metrics, and cloud testing

**Main Takeaway**: The application is ready for production deployment under expected load conditions. Local tests confirm strong core performance, while cloud tests revealed infrastructure considerations (ngrok limitations) that would need addressing for real-world scaling.
...
- **Hands-on Learning**: Actually running tests taught more than any tutorial could
- **Problem-Solving**: Dealing with ngrok errors forced deeper understanding of infrastructure vs application issues
- **Real-World Perspective**: Seeing differences between local and cloud tests highlighted how network factors impact performance

**Beyond the Technical:**

The most important realization was that **good performance is a feature users feel**. Slow applications frustrate users even if the functionality is perfect. Testing ensures we're not just building features, but building experiences that work well under pressure.

These skills, designing tests, interpreting metrics, troubleshooting issues, will be valuable in any future development work. The ability to confidently say "this application can handle X users" based on actual data rather than guesswork is a practical skill I'll use throughout my career.
